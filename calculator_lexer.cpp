...
CalculatorLexer::CalculatorLexer() {
  ...
  _lex.add_token(ADD, make_regex("\\s*\\+\\s*"));
  ...
  _lex.add_token(DISPLAY, make_regex("display"));
   _lex.add_token(POW, make_regex("^"));
   _lex.add_token(SEPARATOR, make_regex(";"));  
  _lex.add_token(SQUAREROOT, make_regex("sroot"));
}
...
Lexer::Token CalculatorLexer::next() {
  ...
  do {
    token = _lex.next();
  } while (std::isspace(token.lexeme[0]));  // Check for whitespace character
  return token;
}